...
const useSortableData = (items: PrescriptionInterface[] = [], config = null) => {
  ...
  const sortedItems = React.useMemo(() => {
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a: PrescriptionInterface, b: PrescriptionInterface) => {
...
export const PrescriptionHistoryTable = (props: { data: PrescriptionInterface[] }) => {
  ...
  const handleOrderNumber = (Order_Number: string, Rx_id: string) => {
    router(`/home/prescriptions/order/${Order_Number}-${Rx_id}`);
  };

  useEffect(() => {
    setSortConfig({ key: 'Issue_Date', direction: 'descending' });
  }, []);
      ...
      <Table sx={{ minWidth: 700, minHeight: 300 }} stickyHeader>
        <TableHead className="table_head">
          <TableRow>
            ...
            <StyledTableCell onClick={() => requestSort('Patient_Name')} className={getClassNamesFor('Patient_Name')} sx={{ display: 'flex', justifyContent: 'left' }}>
              Patient{' '}
          ...
          {items &&
            items.map((row: PrescriptionInterface, index: number) => (
              <StyledTableRow key={index}>
                ...
                <StyledTableCell component="td" className="table_first" onClick={() => handleOrderNumber(row.Order_id, row.Rx_id)}>
                  {/* <Stack>{row.Order_id ? <Box className="table_first">ORD-{row.Order_id}</Box> : null}</Stack> */}
