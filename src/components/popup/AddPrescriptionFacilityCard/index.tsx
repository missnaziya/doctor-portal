import { addPrescribeFacilityCardForm, AddPrescribeFacilityCardFormHandler } from '../../../services/pages/popup-form/AddPrescribeFacilityCardForm.service';
...
export const AddPrescriptionFacilityCard = (props: { handleClose: (res: boolean) => void}) => {

  const handleCard = async () => {
    const res = await AddPrescribeFacilityCardFormHandler();
    if (res === 'OK') props.handleClose(true);
    else props.handleClose(false);
  };
            ...
            <Box className="main-box">
              <Box sx={{ bgcolor: 'background.paper' }}>
                <Stack className="modal_heading_main" direction="row" justifyContent="center" alignItems="center">
                  <Typography className="heading_bottom_without_border">Add Card</Typography>
                </Stack>
                <FieldGroup
                  control={addPrescribeFacilityCardForm}
                  render={() => (
                    <form>
                      <Box>
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={12} sm={12} md={12} lg={12} mt={3}>
                            <Stack className="add_btnouter" direction="row" justifyContent="center" alignItems="center" gap={1.5}>
                              <Box className="add_outerbtnscancel" onClick={() => props.handleClose(false)}>
                                <Button
